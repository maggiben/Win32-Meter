...
///////////////////////////////////////////////////////////////////////////////////////////
// Returns the list's next available element                                             //
///////////////////////////////////////////////////////////////////////////////////////////
...
LRESULT WINAPI meterProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DESTROY:
		...
		PostQuitMessage(0);
		break;
	//NULL paint: just refreshes old data no without any move  
	case WM_PAINT:
		{
			myMeter = GetMeter(hWnd);
			PAINTSTRUCT ps;
	...
	case WM_WINDOWPOSCHANGED:
			{
				...
				////////////////////////////////////////////
				//realloc more memory for the grid style  //
				////////////////////////////////////////////
	...
	case WM_SETGDSPACING:
		...
		break;	
	//Filled Style for Grid Like control
	case WM_FILLED:
		myMeter = GetMeter(hWnd);
	...
	case WM_PEAK:
		...
		break;
	//Set Max peak signal Color
	case WM_PEAKCOLOR:
		myMeter = GetMeter(hWnd);
		myMeter->peakColor = (COLORREF)wParam;
		break;
	case WM_SETFONT:
		{
			myMeter = GetMeter(hWnd);
	...
	case WM_NCPAINT:
		{
			...
			//Calculate the border (1px)
			rcBorder.right	-= rcBorder.left; 
			rcBorder.bottom -= rcBorder.top;
			rcBorder.left	 = rcBorder.top = 0;
			
			//If no color draw a 3D edge else paint a solid frame
			if(!myMeter->bdColor) {
			...
			//Calculate the edge size and paint it !
...
void makeGaugeEx(HWND hwnd, LPMETER_OPTIONS options, float newP,BOOL update)
{
	...
	if(update)
	{
		...
		if(options->peak)
		if(options->mPeak < percent){
		   options->mPeak = (newP / 100) * (HEIGHT - (border + sizTxt.cy));
		}
		else {
			tmp = options->mPeak;
			tmp -= ((options->mPeak - percent) / 10);										//Nice fall-off effect 
			if(tmp >= percent) {options->mPeak = tmp;} else {options->mPeak = percent;}
		}	
	}
	else
	{
	...
	for(int y = border; y <= HEIGHT - (border + sizTxt.cy) ; y+=4)				//options->grid_space Vertical Lines
	{
		...
		if(options->peak)
		if( 
		    (y > (( HEIGHT - (border + sizTxt.cy) ) - options->mPeak) - 2 )
	    &&  (y < (( HEIGHT - (border + sizTxt.cy) ) - options->mPeak) + 3 )
	      )
		{
			DeleteObject(SelectObject(hdcMem, colorPen));
...
void makeGridEx(HWND hwnd, LPMETER_OPTIONS options, float newP,BOOL update)
{
	...
	for(int x = 0; x <= WIDTH + offset; x+=2)
	{
		for(int y = 0; y <= HEIGHT; y+=2)
		{
			SetPixel(hdcMem,x,y,RGB(25,25,25)); // simply dot
		}
	}
	...
	for(int x = 0; x <= WIDTH + offset; x+= options->grid_space)
	{
		for(int y = 0; y <= HEIGHT; y+= options->grid_space)
		{
			SetPixel(hdcMem,x,y,RGB(200,200,200)); // simply dot
		}
	}
	//Delete the grid pen
	DeleteObject(SelectObject(hdcMem, colorPen));
	...
	colorPen = SelectObject(hdcMem,CreatePen(PS_SOLID,1,options->fgColor));	// Select the pen for the data in this case is 1px solid
	cnt = 0;
	...
	for(int x = offset; x < WIDTH + offset; x++)
	{
		if(options->data != NULL){		
			...
			{
				///////////////////////////////////////////////
				//Heavy ilteration dont run on slow machines //
				///////////////////////////////////////////////
				if(options->fgColor2 != options->fgColor)
				{
				...
				// Easy fill only one color less CPU cycles
				else
				{
					MoveToEx(hdcMem, x, HEIGHT, NULL);									// Easy fill with one color only
	...
	if(offset == WIDTH) { offset = 0;}														// Reset the offset
	if(update){offset++;}																	// Incremet the offset only if updating !
	options->offset = offset;
	////////////////////////////////
	// Release all resouces	taken //
	////////////////////////////////
	DeleteObject(SelectObject(hdcMem, colorPen));
	DeleteDC(hdcMem);
	DeleteObject(hbmp);
