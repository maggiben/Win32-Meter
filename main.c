...
static LRESULT CALLBACK MainDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
			...
			SendMessage(myControl2,WM_SETGDSPACING, (WPARAM)gridSize, (LPARAM)NULL);
			*/
			SendMessage(myControl2,WM_SETFGCOLOR, (WPARAM)RGB(0,255,0), (LPARAM)NULL);
			SendMessage(myControl2,WM_SETGDCOLOR, (WPARAM)RGB(25,25,25), (LPARAM)NULL);
			SendMessage(myControl2,WM_FILLED,(WPARAM)FALSE,(LPARAM)NULL);
			SendMessage(myControl2,WM_SETGDSPACING, (WPARAM)gridSize, (LPARAM)NULL);
			...
			menu_button = CreateWindow("button", NULL, WS_CHILD | WS_VISIBLE | BS_PUSHLIKE |BS_OWNERDRAW,
				...
				75,
				22,
			hwndDlg, (HMENU)(331), ghInstance, NULL);
			buttons_proc = (WNDPROC)SetWindowLong(menu_button,GWL_WNDPROC, (DWORD)ButtonProc);
			
			//MessageBox(NULL,"Error loading user32.dll","Oops Error !", IDOK);
...
LRESULT CALLBACK ButtonProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		default:
			return CallWindowProc(buttons_proc, hWnd, message, wParam, lParam) ;
	}
